...
public class LeafCommand implements TurtleCommand {
  ...
  public LeafCommand(
      Color leaf1Color,
      Color leaf2Color,
      double colorCenterOffset,
      double color2Offset,
      double leafSize,
      double leafThicknessFactor,
      double leafLengthFactor,
      double leafWidthFactor,
      double widthAngle
  ) {
  ...
  public void execute(Deque<TurtleState> turtleStates, GraphicsContext gc, Group world) {
    ...
    RadialGradient radialGradient = new RadialGradient(
        0,
        0,
        state.x + dxColorCenter,
        state.y + dyColorCenter,
        length,
        false,
        CycleMethod.NO_CYCLE,
        new Stop(0.0, leaf1Color),
        new Stop(color2Offset, leaf2Color)
    );
